...
async function requireAdmin() {
    const access = await requireAdminRouteAccess("privacy")
...
export async function getPrivacyAdminData(): Promise<DocumentPageContent> {
    const supabase = await requireAdmin()
    const { data, error } = await supabase
        .from("app_settings")
        .select("value")
        .eq("key", PRIVACY_PAGE_SETTING_KEY)
        .maybeSingle()
    ...
    return normalizeDocumentPageContent(data?.value ?? DEFAULT_PRIVACY_PAGE_CONTENT, DEFAULT_PRIVACY_PAGE_CONTENT)
}

export async function savePrivacyPageContent(input: DocumentPageContent) {
    const supabase = await requireAdmin()
    ...
    const { error } = await supabase
        .from("app_settings")
        .upsert({
            key: PRIVACY_PAGE_SETTING_KEY,
            value,
            description: "Editable public privacy policy page content.",
        })

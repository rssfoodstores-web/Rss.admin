    ...
    return (
        <div className="space-y-6">
            ...
            {shownTab === "settings" && initialData.access.canManageSettings ? (
                <div className="space-y-6">
                    {initialData.connection?.webhookUrl ? <SectionCard><p className="font-black text-emerald-950 dark:text-emerald-100">Receive customer replies in RSS</p><p className="mt-1 text-sm text-gray-500">In WhatChimp, open Bot Manager → your WhatsApp number → Configuration. Turn on “Trigger Webhook for Incoming Message,” replace any old URL with this complete URL, then click Save Settings. The outgoing-message webhook is a different setting.</p><Input className="mt-4" readOnly value={initialData.connection.webhookUrl} onFocus={(event) => event.currentTarget.select()} /><p className="mt-2 text-xs text-amber-700">Treat this URL like a password. Only a Supa Admin should copy it into WhatChimp.</p></SectionCard> : null}

...
export default async function AccountInfoPage() {
    ...
    const agentStatusMap = new Map<string, string | null>()
    const merchantStatusMap = new Map<string, string | null>()
    ...
    const administrativeSeats = users.filter((user) => user.role === "admin" || user.role === "supa_admin" || user.role === "sub_admin").length
    const networkPartners = users.filter((user) => user.role === "agent" || user.role === "rider").length

    return (
        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-8">
            <div>
                <h1 className="text-3xl font-bold tracking-tight text-foreground">Account Info</h1>
                <p className="text-muted-foreground mt-1 text-base">
                    View user account details, profile status, roles, points, and exports.
                </p>
            </div>
